...
function resetRecordForm() {
    document
        .querySelectorAll("#recordScreen .emotion-option")
        .forEach((option) => {
            option.classList.remove("selected");
        });
...
function saveEmotion() {
    const selectedEmotion = document.querySelector(
        "#recordScreen .emotion-option.selected"
    );
    ...
    listContainer.innerHTML = records
        .map((record) => {
            ...
            <div class="emotion-item" onclick="openEditModal('${record.id}')">
                ...
                ${
                    record.comment
                        ? `<p class="emotion-item-comment">${record.comment}</p>`
                        : ""
                }
            </div>
        `;
...
function updateEmotion() {
    const selectedEmotion = document.querySelector(
        "#editModal .emotion-option.selected"
    );
    ...
    records[recordIndex].comment = document
